...
def compare_train_val_accuracy(file_label_map: Dict[str, str], fig_dir: str = "./Compare/mu/alpha_1") -> None:
    ...
    for i, label in enumerate(sorted_labels, 1):
        ...
        plt.title(f"Accuracy Comparison μ={label.split('=')[1]} (α=0.1)")
        plt.xlabel("Round")
        plt.ylabel("Accuracy")
        plt.legend()
        plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, "accuracy_comparison_mu_with_lower_alpha.png"))
    plt.close()


if __name__ == "__main__":
    file_label_map = {
        "result_alpha_1_mu_01.json": "μ=0.1",
        "result_alpha_1_mu_05.json": "μ=0.5",
        "result_alpha_1_mu_1.json": "μ=1"
    }
