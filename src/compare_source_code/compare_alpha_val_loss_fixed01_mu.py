...
def compare_alpha_results(file_label_map: Dict[str, str], fig_dir: str = "./Compare/alpha") -> None:
    ...
    plt.title("Validation Accuracy Comparison (μ=0.1)")
    plt.xlabel("Round")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True)
    ...
    plt.title("Validation Loss Comparison (μ=0.1)")
    plt.xlabel("Round")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, "alpha_comparison_fixed_mu.png"))
    plt.close()


if __name__ == "__main__":
    file_label_map = {
        "result_alpha_10_mu_01.json": "α=10.0",
        "result_alpha_1_mu_01.json": "α=1.0",
        "result_alpha_01_mu_01.json": "α=0.1"
    }

    compare_alpha_results(file_label_map)