...
def compare_train_val_accuracy(file_label_map: Dict[str, str], fig_dir: str = "./Compare/alpha") -> None:
    ...
    for i, label in enumerate(sorted_labels, 1):
        ...
        plt.title(f"Accuracy Comparison α={label.split('=')[1]} (μ=0.1)")
        plt.xlabel("Round")
        plt.ylabel("Accuracy")
        plt.legend()
        plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, "train_val_accuracy_comparison_alpha_fixed_mu.png"))
    plt.close()
