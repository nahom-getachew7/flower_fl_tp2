...
def compare_multiple_results(file_label_map: Dict[str, str], fig_dir: str = "Comparison_figures") -> None:
    full_fig_dir = os.path.join(BASE_DIR, fig_dir, "alpha_1")
    ...
    plt.title("Training Accuracy Comparison (α=1)")
    plt.xlabel("Round")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True)
    ...
    plt.title("Validation Accuracy Comparison (α=1)")
    plt.xlabel("Round")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True)
...
if __name__ == "__main__":
    file_label_map = {
        "FedAvg_alpha_1.json": "FedAvg",
        "FedProx_alpha_1_mu_01.json": "FedProx(μ=0.1)",
        "SCAFFOLD_alpha_1.json": "SCAFFOLD"
    }
